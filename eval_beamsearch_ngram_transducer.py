...
def decoding_step(
    ...
    for batch_idx in it:
        ...
        # Collect references and hypotheses for the batch
        batch_refs = target_transcripts[sample_idx : sample_idx + len(probs_batch)]
        batch_hyps = []
        batch_best_hyps = []

        for beams_idx, beams in enumerate(beams_batch):
            target = batch_refs[beams_idx]
            target_split_c = list(target)  # For CER
            ...
            for candidate_idx, candidate in enumerate(beams):  # type: (int, rnnt_beam_decoding.rnnt_utils.Hypothesis)
                ...
                pred_split_c = list(pred_text)  # For CER
                cer_dist = editdistance.eval(target_split_c, pred_split_c)

                if candidate_idx == 0:
                    # First candidate (for wer_dist_first, cer_dist_first)
                    batch_hyps.append(pred_text)
                    cer_dist_first += cer_dist
                batch_best_hyps.append(pred_text)  # Collect all hypotheses for oracle WER
        ...
        # Compute WER for the first hypothesis using jiwer
        if batch_hyps:
            wer = jiwer.wer(
                batch_refs,
                batch_hyps,
            ...
            # Accumulate WER for first hypothesis (equivalent to wer_dist_first)
            wer_dist_first += wer * len(batch_refs)  # Weighted by batch size
            words_count += len(batch_refs)  # Approximate, as jiwer handles word counting internally

        # Compute oracle WER (best hypothesis) using jiwer
        if batch_best_hyps:
            # For oracle WER, select the best hypothesis for each reference
            best_wer = float('inf')
            for hyp in batch_best_hyps:
                wer = jiwer.wer(
                    batch_refs,
                    [hyp] * len(batch_refs),  # Compare each hypothesis against all references
                    truth_transform=jiwer_transform,
                    hypothesis_transform=jiwer_transform
                )
                best_wer = min(best_wer, wer)
            wer_dist_best += best_wer * len(batch_refs)
    ...
    # Normalize WER and CER
    wer_first = wer_dist_first / words_count if words_count > 0 else 0
    cer_first = cer_dist_first / chars_count if chars_count > 0 else 0
    wer_best = wer_dist_best / words_count if words_count > 0 else 0
    cer_best = cer_dist_best / chars_count if chars_count > 0 else 0

    if cfg.decoding_strategy == "greedy_batch":
        logging.info(f"Greedy batch WER/CER = {wer_first:.2%}/{cer_first:.2%}")
        return wer_first, cer_first

    if cfg.decoding.ngram_lm_model:
        logging.info(
            f"WER/CER with {cfg.decoding_strategy} decoding and N-gram model = {wer_first:.2%}/{cer_first:.2%}"
        )
    else:
        logging.info(
            f"WER/CER with {cfg.decoding_strategy} decoding = {wer_first:.2%}/{cer_first:.2%}"
        )
    logging.info(
        f"Oracle WER/CER in candidates with perfect LM = {wer_best:.2%}/{cer_best:.2%}"
    )
    logging.info(f"=================================================================================")
